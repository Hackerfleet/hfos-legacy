...
/**
 * @ngdoc function
 * @name hfosFrontendApp.controller:contactsCtrl
 * @description
 * # contactsCtrl
...
class contacts {
    ...
    constructor(scope, rootscope, schemata, $modal, user, objectproxy, socket, menu, NgTableParams, common) {
        ...
        this.request_contacts = function () {
            console.log('[CONTACTS] Login successful - fetching contact data');
            this.op.searchItems('contact', '*', '*').then(function (contacts) {
        ...
        //self.socket.listen('hfos.auth.login', self.request_contacts);

        this.loginupdate = this.rootscope.$on('User.Login', function () {
            console.log('[ROSTER] Just Logged in, getting contacts');
            self.request_contacts();
        });

        if (this.user.signedin === true) {
            console.log('[ROSTER] Logged in, getting contacts');
...
contacts.$inject = ['$scope', '$rootScope', 'schemata', '$modal', 'user', 'objectproxy', 'socket', 'menu', 'NgTableParams'];
