...
LayerSchema = {
    ...
    'properties': {
        ...
        'layerOptions': {
            'minZoom': {'type': 'number', 'description': 'Minimum zoom number.', 'default': 0},
            'maxZoom': {'type': 'number', 'description': 'Maximum zoom number.', 'default': 18},
            'maxNativeZoom': {'type': 'number',
                              'description': 'Maximum zoom number the tiles source has available. If it is specified,\
                               the tiles on all zoom levels higher than maxNativeZoom will be loaded from maxZoom level\
                               and auto-scaled.',
                              'default': 'null'},
            ...
            'attribution': {'type': 'string',
                            'description': 'e.g. "&copy; Openstreetmap" - the string used by the attribution control,\
            ...
            'continuousWorld': {'type': 'boolean',
                                'description': 'If set to true, the tile coordinates won''t be wrapped by world width\
                                 (-180 to 180 longitude) or clamped to lie within world height (-90 to 90). Use this\
                                 if you use Leaflet for maps that don''t reflect the real world (e.g. game, indoor or\
                                 photo maps).',
                                'default': False},
            'noWrap': {'type': 'boolean',
                       'description': 'If set to true, the tiles just won''t load outside the world width (-180 to\
            ...
            'zoomReverse': {'type': 'boolean',
                            ...
                            'default': False},
            'opacity': {'type': 'number', 'description': 'The opacity of the tile layer.', 'default': 1.0},
            'zIndex': {'type': 'number', 'description': 'The explicit zIndex of the tile layer. Not set by default.',
                       'default': 'null'},
            'unloadInvisibleTiles': {'type': 'boolean',
                                     'description': 'If true, all the tiles that are not visible after panning are\
                                      removed (for better performance). true by default on mobile WebKit, otherwise\
                                      false.'},
            'updateWhenIdle': {'type': 'boolean',
                               'description': 'If false, new tiles are loaded during panning, otherwise only after it\
                                (for better performance). true by default on mobile WebKit, otherwise false.'},
            'detectRetina': {'type': 'boolean',
                             'description': 'If true and user is on a retina display, it will request four tiles of\
                              half the specified size and a bigger zoom level in place of one to utilize the\
                              high resolution.'},
            'reuseTiles': {'type': 'boolean',
                           'description': 'If true, all the tiles that are not visible after panning are placed in a\
                            reuse queue from which they will be fetched when new tiles become visible (as opposed to\
                            dynamically creating new ones). This will in theory keep memory usage low and eliminate\
                            the need for reserving new memory whenever a new tile is needed.'},
            'bounds': {'type': 'list',
