...
try:
    ...
    publish_parts = None
    hfoslog("[WIKI] No docutils found! Install it to get full functionality!", lvl=warn)
    ...
    def objectchange(self, event):
        ...
        try:
            if event.schema == 'wikipage':
                newpage = objectmodels['wikipage'].find_one({'uuid': event.uuid})
                try:
                    hfoslog('[WIKI] Got a rendering candidate: ', newpage._fields)

                    try:
                        if not publish_parts:
                            html = newpage.text
                        else:
                            html = publish_parts(newpage.text, writer_name='html')['html_body']
                        newpage.html = html
                        newpage.save()
                        hfoslog("[WIKI] Page successfully rendered")
                    except Exception as e:
                        hfoslog("[WIKI] Error during html rendering: ", e, type(e), newpage.isbn, lvl=error)
                except Exception as e:
                    hfoslog("[WIKI] Error during page update.")

        except Exception as e:
            hfoslog("[WIKI] Page rendering notification error: ", event, e, type(e), lvl=error)
